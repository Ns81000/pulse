...
export const Route = createFileRoute("/guide")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Live program guide across the IPTV catalog. Timeline of what's playing right now." },
...
function GuidePage() {
  ...
  const countriesWithEpg = useMemo(() => {
    if (!cat.data || !epg.data) return [] as { code: string; name: string; flag: string; count: number }[];
  ...
  const channels = useMemo(() => {
    if (!cat.data || !epg.data) return [] as { id: string; name: string; logo: string | null; now: { title: string; start: Date; end: Date }; next: { title: string; start: Date; end: Date } | null }[];
    const base = country ? (cat.data.indexes.by_country[country] ?? []) : cat.data.indexes.all_ids;
    const out: { id: string; name: string; logo: string | null; now: { title: string; start: Date; end: Date }; next: { title: string; start: Date; end: Date } | null }[] = [];
    for (const id of base) {
      const p = epg.data.programs[id];
      if (!p?.now) continue;
      const ch = cat.data.channels[id]; if (!ch) continue;
      out.push({
        id, name: ch.name, logo: ch.logo_url,
        now: { title: p.now.title, start: new Date(p.now.start), end: new Date(p.now.end) },
        next: p.next ? { title: p.next.title, start: new Date(p.next.start), end: new Date(p.next.end) } : null,
      });
  ...
  const hours = Array.from({ length: 7 }, (_, i) => {
    const d = new Date(windowStart); d.setHours(d.getHours() + i); return d;
  });
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">Live guide</h1>
          ...
          <div className="flex">
            ...
            <div className="shrink-0 border-r border-[var(--border-subtle)]">
              ...
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="flex h-16 w-[180px] items-center gap-2.5 border-b border-[var(--border-subtle)] px-3 sm:w-[220px]">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <div className="flex h-10 border-b border-[var(--border-subtle)] bg-[var(--surface-2)]">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="flex items-center border-r border-[var(--border-subtle)] px-3" style={{ minWidth: "180px" }}>
              ...
              {/* Program block rows */}
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="flex h-16 items-center gap-2 border-b border-[var(--border-subtle)] px-3">
                  <div className="shimmer h-9 rounded-md" style={{ width: `${140 + (i % 4) * 60}px` }} />
                  <div className="shimmer h-9 rounded-md" style={{ width: `${100 + (i % 3) * 50}px` }} />
                  <div className="shimmer h-9 rounded-md" style={{ width: `${120 + (i % 2) * 40}px` }} />
                </div>
              ))}
            </div>
          </div>
      ...
      {empty && (
        <div className="grid place-items-center rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-6 py-16 text-center">
          ...
          <p className="mt-3 text-[14px] text-[var(--text-secondary)]">No program data for this selection.</p>
          <p className="mt-1 max-w-sm text-[12px] text-[var(--text-tertiary)]">EPG coverage from iptv-org varies by region. Try another country or browse all channels.</p>
          <button onClick={() => navigate({ to: "/browse" })} className="btn-primary mt-4">Browse channels</button>
        </div>
      )}
              ...
              {channels.map((c) => (
                ...
                >
                  <div className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-md bg-[var(--surface-base)]">
                    {c.logo ? <img src={c.logo} alt="" className="max-h-[80%] max-w-[80%] object-contain" /> : <Tv className="size-4 text-[var(--text-disabled)]" />}
                  </div>
            ...
            <div ref={scrollRef} className="relative flex-1 overflow-x-auto">
              <div className="relative" style={{ width: HOUR_PX * 7 }}>
                ...
                <div className="sticky top-0 z-10 flex h-10 border-b border-[var(--border-subtle)] bg-[var(--surface-2)]">
                  {hours.map((h, i) => (
                    <div key={i} className="border-r border-[var(--border-subtle)] px-3 py-2 font-mono text-[11px] text-[var(--text-tertiary)]" style={{ width: HOUR_PX }}>
                      {h.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </div>
                  ))}
                </div>
                {/* Now line */}
                <div className="pointer-events-none absolute bottom-0 top-10 z-20 w-px bg-[var(--accent)]" style={{ left: nowX }}>
                  <span className="absolute -left-[5px] -top-1 size-[11px] rounded-full bg-[var(--accent)]" />
                </div>
                {/* Rows */}
                {channels.map((c) => {
                  const blocks = [c.now, c.next].filter(Boolean) as { title: string; start: Date; end: Date }[];
                  return (
                    <div key={c.id} className="relative h-16 border-b border-[var(--border-subtle)]">
                      {blocks.map((b, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="truncate text-[10px] text-[var(--text-tertiary)]">{b.start.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                          </button>
                        );
                      })}
                    </div>
