...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse and watch thousands of free IPTV channels. Verified live at every click — no dead ends." },
      { property: "og:title", content: "Tela — Watch the world live" },
      { property: "og:description", content: "Browse and watch thousands of free IPTV channels. Verified live at every click." },
    ],
  }),
  component: Index,
});
...
function Shelf({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mt-12">
      <h2 className="mb-4 font-mono text-[10.5px] font-medium uppercase tracking-[0.18em] text-[var(--text-tertiary)]">{title}</h2>
      {children}
    </section>
  );
}
...
function Index() {
  ...
  const onNowIds = useMemo(() => {
    ...
    for (const id of cat.data.indexes.all_ids) {
      if (epg.data.programs[id]?.now) { out.push(id); if (out.length >= 14) break; }
    }
    return out;
  }, [cat.data, epg.data]);
    ...
    <div>
      <section className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-1)] px-6 py-14 sm:px-12 sm:py-20">
        <div className="relative z-10 max-w-2xl">
          <p className="font-mono text-[10.5px] font-medium uppercase tracking-[0.22em] text-[var(--text-tertiary)]">Free · No account · Verified live</p>
          <h1 className="mt-4 font-display text-4xl font-semibold tracking-tight sm:text-6xl" style={{ letterSpacing: "-0.035em" }}>
            Watch the world,<br/><span className="text-[var(--text-tertiary)]">live.</span>
          </h1>
          <p className="mt-5 max-w-xl text-[15px] leading-relaxed text-[var(--text-secondary)]">
            Thousands of public IPTV channels — news, sports, films, music — from every continent. Every channel is verified the instant you click. No dead ends.
          </p>
          <div className="mt-7 flex flex-wrap gap-2">
            <Link to="/browse" className="btn-primary inline-flex items-center gap-1.5">Browse channels <ArrowRight className="size-3.5" /></Link>
            <Link to="/guide" className="btn-ghost">Live guide</Link>
          </div>
        ...
        <>
                ...
                {onNowIds.map((id) => {
                  ...
                  return (
                    <div key={id} className="w-[210px] shrink-0">
                      <ChannelCard channel={c} flag={featured.flagBy.get(c.country)} epg={epg.data?.programs[id]} categoryName={featured.catName} isFavourite={favSet.has(id)} onFavouriteChange={refreshFavs} />
                    </div>
                  );
                })}
              </HorizScroll>
          ...
          {popularByCategory.map((shelf) => (
            <Shelf key={shelf.id} title={shelf.name}>
              <HorizScroll>
                {shelf.ids.map((id) => {
                  const c = cat.data!.channels[id]; if (!c) return null;
                  return (
                    <div key={id} className="w-[210px] shrink-0">
                      <ChannelCard channel={c} flag={featured.flagBy.get(c.country)} epg={epg.data?.programs[id]} categoryName={featured.catName} isFavourite={favSet.has(id)} onFavouriteChange={refreshFavs} />
                    </div>
