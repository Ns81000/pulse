...
async function check(url: string, referrer: string | null, ua: string | null): Promise<Status> {
  ...
  try {
    ...
    const resp = await fetch(url, { method: "GET", headers, signal: controller.signal, redirect: "follow" });
    ...
    const looksLikeStream =
      ...
      ct.includes("mp2t");
    return looksLikeStream ? "online" : "online"; // assume online if 2xx — many CDNs hide type
  } catch (e: any) {
    clearTimeout(t);
